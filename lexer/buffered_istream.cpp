...
char BufferedIStream::peek()
{
    return (char)input_stream.peek();
}

[[maybe_unused]] std::string BufferedIStream::get_buffer()
{
    return buffer;
}

void BufferedIStream::add_to_buffer(char c)
{
    buffer.push_back(c);
}

[[maybe_unused]] void BufferedIStream::clear_buffer()
{
    buffer.clear();
}
...
unsigned int BufferedIStream::get_current_line() const
{
    return current_line;
}
...
[[nodiscard]] bool BufferedIStream::eof() const
{
    return is_eof;
}
