  ...
  case '$':
    if (try_consume([](char c) { return is_digit(c, 16); })) {
      ...
      return prepare_token(TokenType::LITERAL_INTEGER,
                           get_integer_value(get_buffer(), 16), get_buffer());
    }
    break;
  case '&':
    if (try_consume([](char c) { return is_digit(c, 8); })) {
      ...
      return prepare_token(TokenType::LITERAL_INTEGER,
                           get_integer_value(get_buffer(), 8), get_buffer());
    }
    break;
  case '%':
  ...
  default:
    if (is_digit(buffer_peek())) {
      ...
      if (type == TokenType::LITERAL_INTEGER) {
        return prepare_token(type, get_integer_value(get_buffer(), 10),
                             get_buffer());
      } else {
    ...
    if (is_start_of_identifier(buffer_peek())) {
      ...
      if (value_in_keyword_enum.has_value()) {
        return prepare_token(TokenType::KEYWORD, value_in_keyword_enum.value(),
                             get_buffer());
      }
