...
Token Scanner::next_token()
{
    ...
    case '+':
        if (try_consume('='))
        {
            return prepare_token(TokenType::OPER, Operators::ADDASSIGN, get_buffer());
        }
        return prepare_token(TokenType::OPER, Operators::ADD, get_buffer());
    case '-':
        if (try_consume('='))
        {
            return prepare_token(TokenType::OPER, Operators::SUBASSIGN, get_buffer());
        }
        return prepare_token(TokenType::OPER, Operators::SUB, get_buffer());
    case '*':
        if (try_consume('='))
        {
            return prepare_token(TokenType::OPER, Operators::MULASSIGN, get_buffer());
        }
        return prepare_token(TokenType::OPER, Operators::MUL, get_buffer());
    case '/':
        if (try_consume('='))
        {
            return prepare_token(TokenType::OPER, Operators::QUOASSIGN, get_buffer());
        }
        return prepare_token(TokenType::OPER, Operators::QUO, get_buffer());
    case '<':
        if (try_consume('>'))
        {
            return prepare_token(TokenType::OPER, Operators::NEQ, get_buffer());
        }
        else if (try_consume('='))
        {
            return prepare_token(TokenType::OPER, Operators::LEQ, get_buffer());
        }
        return prepare_token(TokenType::OPER, Operators::LES, get_buffer());
    case ':':
        if (try_consume('='))
        {
            return prepare_token(TokenType::OPER, Operators::ASSIGN, get_buffer());
        }
        return prepare_token(TokenType::SEPERATOR, Separators::COLON, get_buffer());
    case '>':
        if (try_consume('='))
        {
            return prepare_token(TokenType::OPER, Operators::GEQ, get_buffer());
        }
        return prepare_token(TokenType::OPER, Operators::GTR, get_buffer());
    case '=':
        return prepare_token(TokenType::OPER, Operators::EQL, get_buffer());
    case '^':
        return prepare_token(TokenType::OPER, Operators::DEREF, get_buffer());
    case '(':
        return prepare_token(TokenType::SEPERATOR, Separators::LPAREN, get_buffer());
    case ')':
        return prepare_token(TokenType::SEPERATOR, Separators::RPAREN, get_buffer());
    case '[':
        return prepare_token(TokenType::SEPERATOR, Separators::LBRACK, get_buffer());
    case ']':
        return prepare_token(TokenType::SEPERATOR, Separators::RBRACK, get_buffer());
    case ',':
        return prepare_token(TokenType::SEPERATOR, Separators::COMMA, get_buffer());
    case ';':
        return prepare_token(TokenType::SEPERATOR, Separators::SEMICOLON, get_buffer());
    case '.':
        if (try_consume('.'))
        {
            return prepare_token(TokenType::SEPERATOR, Separators::ELLIPSIS, get_buffer());
        }
        return prepare_token(TokenType::SEPERATOR, Separators::PERIOD, get_buffer());
    case '\'':
    ...
    case '$':
        if (try_consume([](char c) { return is_digit(c, 16); }))
        {
            ...
            return prepare_token(TokenType::LITERAL_INTEGER, get_integer_value(get_buffer(), 16), get_buffer());
        }
        break;
    case '&':
        if (try_consume([](char c) { return is_digit(c, 8); }))
        {
            ...
            return prepare_token(TokenType::LITERAL_INTEGER, get_integer_value(get_buffer(), 8), get_buffer());
        }
        break;
    case '%':
        if (try_consume([](char c) { return is_digit(c, 2); }))
        {
            ...
            return prepare_token(TokenType::LITERAL_INTEGER, get_integer_value(get_buffer(), 2), get_buffer());
        }
        break;
    case '#':
    ...
    default:
        if (is_digit(buffer_peek()))
        {
            ...
            assert(type == TokenType::LITERAL_INTEGER || type == TokenType::LITEREAL_DOUBLE);

            if (type == TokenType::LITERAL_INTEGER)
            {
                return prepare_token(type, get_integer_value(get_buffer(), 10), get_buffer());
            }
            else
            {
                return prepare_token(type, get_double_value(get_buffer()), get_buffer());
            }
        }
        if (is_start_of_identifier(buffer_peek()))
        {
            ...
            std::transform(buffer_in_upper.begin(), buffer_in_upper.end(), buffer_in_upper.begin(), ::toupper);

            auto value_in_keyword_enum = magic_enum::enum_cast<Keywords>(buffer_in_upper);

            if (value_in_keyword_enum.has_value())
            {
                return prepare_token(TokenType::KEYWORD, value_in_keyword_enum.value(), get_buffer());
            }
...
Token Scanner::scan_string_literal(bool start_with_hash)
{
    ...
    while (current_state != finish)
    {
        ...
        case pre_read_number:
            ...
            {
                number_of_special_symbol = number_of_special_symbol * 10 + buffer_peek() - '0';
                current_state = read_number;
            }
            else
            {
                throw IllegalCharacterException(get_current_line(), get_current_column(), peek());
            }
            break;
        case read_number:
            if (try_consume([](char c) { return is_digit(c, 10); }))
...
TokenType Scanner::scan_number_literal(int numeral_system)
{
    ...
    while (current_state != finish)
    {
        ...
        case number:
            ...
            else if (c == '.' && try_consume([](char c) { return is_digit(c, 10); }))
            { // check next
                current_state = state::number_after_dot;
                type = TokenType::LITEREAL_DOUBLE;
            }
            else if (c == 'e' && try_consume([](char c) { return is_digit(c, 10); }))
            {
...
Token Scanner::prepare_token(TokenType type, const TokenValue &value, const std::string &raw_value) const
{
